...
$\hat p_c = \text{softmax}(c,W_{C\text{x}(q+1)}, x_i)=\frac{e^{w_c'x_i}}{\sum_{c=1}^{C} e^{w_c'x_i}}$
...
# [array([[-0.60904866,  0.4795451 ],
#        [-0.61813617,  1.4820436 ]], dtype=float32)]
```
...
p_hat = Z_exp / Z_exp_sum

p_hat[:10, :]
```

```{python, echo=FALSE}
W = np.array([[-0.60904866,  0.4795451 ],
              [-0.61813617,  1.4820436 ]])
Z = np.dot(X, W)
Z_exp = np.exp(Z)
Z_exp_sum = Z_exp.sum(axis=1)[:, None]
p_hat = Z_exp / Z_exp_sum
