...
```{r, echo=FALSE}
tibble(
  name = c("missing", "imbalance", "engineering", "tuning", "cutoff", "test"),
  df = list(ok_tr_missing, ok_tr_imbalance,
            ok_tr_eng, ok_tr_tuning, ok_tr_cutoff, okcupid_test)) %>%
...
```{r Ok-Miss4, out.width="100%", fig.asp=0.5, warning=FALSE, message=FALSE}
library(ggmosaic)
...
Training multiple recipes, bottom-line saving the AUC for each recipe for each split
...
```{r, echo=FALSE}
rec_miss_unk_knn_none <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(contains("essay"), new_role = "discarded") %>%
  step_log(income, offset = 1) %>%
...
rec_miss_mode_knn_none <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(contains("essay"), new_role = "discarded") %>%
  step_log(income, offset = 1) %>%
  step_knnimpute(income,
                 impute_with = imp_vars(
...
rec_miss_unk_none_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
...
rec_miss_mode_none_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(income, starts_with("isna"), new_role = "discarded") %>%
  step_log(starts_with("len"), offset = 1) %>%
  step_normalize(all_numeric(), -has_role("discarded")) %>%
  step_other(all_nominal(), -has_role("discarded"), other = "other1") %>%
...
rec_miss_unk_mean_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
...
rec_miss_mode_mean_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(starts_with("isna"), new_role = "discarded") %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_meanimpute(income) %>%
  step_normalize(all_numeric(), -has_role("discarded")) %>%
...
rec_miss_unk_knn_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(starts_with("isna"), new_role = "discarded") %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_knnimpute(income,
                 impute_with = imp_vars(
...
rec_miss_mode_knn_len <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(starts_with("isna"), new_role = "discarded") %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_knnimpute(income,
                 impute_with = imp_vars(
...
rec_miss_unk_none_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
...
rec_miss_mode_none_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
  update_role(income, new_role = "discarded") %>%
  step_log(starts_with("len"), offset = 1) %>%
  step_normalize(all_numeric(), -has_role("discarded")) %>%
  step_other(all_nominal(), -has_role("discarded"), other = "other1") %>%
...
rec_miss_unk_mean_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_meanimpute(income) %>%
  step_normalize(all_numeric(), -has_role("discarded")) %>%
  step_other(all_nominal(), -has_role("discarded"), other = "other1") %>%
...
rec_miss_mode_mean_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_meanimpute(income) %>%
  step_normalize(all_numeric(), -has_role("discarded")) %>%
  step_other(all_nominal(), -has_role("discarded"), other = "other1") %>%
...
rec_miss_unk_knn_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_knnimpute(income,
                 impute_with = imp_vars(
                   sex, age, height, starts_with("job"),
...
rec_miss_mode_knn_isna <- recipe(pets ~ ., data = ok_tr_missing) %>%
  step_log(income, starts_with("len"), offset = 1) %>%
  step_knnimpute(income,
                 impute_with = imp_vars(
                   sex, age, height, starts_with("job"),
...
Again, see the plot (think of how you'd plot the results of a 2x2x3 experiment).

```{r Ok-Miss12, out.width="70%"}
cv_res_missing %>%
...
```{r Ok-FES1, out.width="70%", echo=FALSE}
cv_splits_eng %>%
    pivot_longer(cols = names(lst_recs),
                 names_to = "recipe", values_to = "AUC") %>%
  select(id, recipe, AUC) %>%
...
print(textfeatures::word_dims("Why would Kim Jong-un insult me by calling me \"old,\""))
```
