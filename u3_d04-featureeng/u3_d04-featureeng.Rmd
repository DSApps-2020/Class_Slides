...
```{r}
print(cv_res_imbalance %>% group_by(recipe) %>% summarise(mean_auc = mean(AUC)))
cv_res_imbalance %>% arrange(-AUC)
```
...
```{r}
recipe(pets ~ ., data = ok_tr_eng %>% select(pets, location)) %>%
  step_mutate(locationA = factor(str_count(location, "l"))) %>%
  step_dummy(locationA) %>%
  prep(ok_tr_eng) %>%
  juice() %>%
...
recipe(pets ~ ., data = ok_tr_eng2 %>% select(pets, essays)) %>%
  update_role(essays, new_role = "discard") %>%
