...
ok_tr_combined <- ok_tr_combined %>%
  bind_cols(
    map_dfr(ok_tr_combined$essays, count_words)
  ) %>%
  select(-starts_with("essay")) %>%
  bind_cols(ok_tr_combined %>% select(essays)) %>%
  mutate_if(is.character, as.factor)
```
...
```{r}
rec_base <- recipe(pets ~ ., data = ok_tr_combined) %>%
  update_role(essays, new_role = "discarded") %>%
  step_textfeature(essays, prefix = "t",
                   extract_functions = my_text_funs) %>%
  step_mutate_at(starts_with("t_"), fn = ~ifelse(is.na(.x), 0, .x)) %>%
...
```{r, echo=FALSE}
all_ints <- read_rds("../data/Ok_interaction_fdr.rds")
```
...
Let's visualize the top pairwise interaction.

```{r W_Dog-Interaction, out.width="100%", fig.asp=0.5}
main_data %>%
  group_by(sex_m, pets) %>%
  summarise(mean_w_dog = mean(w_dog)) %>%
  ungroup() %>%
...
```{r}
# pairwise interactions string code from Max Kuhn's workshop: https://github.com/topepo/FES/blob/master/07_Detecting_Interaction_Effects/7_04_The_Brute-Force_Approach_to_Identifying_Predictive_Interactions/ames_glmnet.R
...
topmain_ints <- topmain_ints %>% 
  as_tibble() %>% 
  mutate(
    term = str_c("starts_with('", var1, "'):starts_with('", var2, "')")
  ) %>% 
  pull(term) %>% 
  paste(collapse = "+")
...
rec_int_topints <- recipe(pets ~ ., data = ok_tr_interaction2) %>%
  ...
  step_dummy(all_nominal(), -all_outcomes(),
             -has_role("discarded"), one_hot = FALSE) %>%
  {if (!"religion_agnosticism.and.laughing.about.it"
       %in% .$var_info$variable)
    step_mutate(., religion_agnosticism.and.laughing.about.it = 0)} %>%
...
```{r, echo=FALSE}
cv_res_int <- read_rds("../data/Ok_int_cv_res_auc.rds")
