...
For example, in `glm()` you can simply specify a `weights`:
...
The authors claim that a hybrid combination of SMOTE and regular down sampling works best, that's why SMOTE is sometimes refered as "hybrid" sampling algo itself
...
- However, if a class output is required, consider output a $k+1$ class: "I don't know"
