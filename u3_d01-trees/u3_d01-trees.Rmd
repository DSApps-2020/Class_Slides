...
predictors <- c("age", "height_cm", "sex", "body_type",
                "diet2", "drinks", "drugs", "religion2",
...
get_validation_set_auc <- function(i, .cp) {
  ...
  mod <- rpart(pets ~ ., data = ok_tr,
               control = rpart.control(cp = .cp, minbucket = 50))
...
```{r Cp-CV-B, out.width = "100%", fig.asp=0.5}
...
mod_trees <- map(1:4, train_tree, .n = nrow(okcupid2_train))
...
```{r, Bagging-LM-CART, out.width="100%", fig.asp=0.5}
