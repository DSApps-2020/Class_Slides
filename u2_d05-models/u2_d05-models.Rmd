...
def degree_hist_with_pl_fit(G, alpha, dmin, xlabel='Degree', ylabel='%Freq or f(d)'):
  deg = [deg for node, deg in G.degree()]
  N, bins, patches = plt.hist(deg, density=True, bins=range(dmin, 1000))
  d = bins
...
def degree_hist_with_pl_fit_log_log(G, alpha, dmin, xlabel='log(Degree)', ylabel='log(%Freq) or log(f(d))'):
  ...
  N, bins = np.histogram(deg, bins = list(np.exp(np.arange(9))))
  d = np.exp(np.arange(1, 9))
  f_d = ((alpha-1)/dmin)*(d/dmin)**(-alpha)
  plt.plot(np.arange(1, 9), np.log(N/len(deg)), 'bo')
  plt.plot(np.arange(1, 9), np.log(f_d), color='red')
  plt.xlabel(xlabel, fontsize=16)
...
.insight[
`r emo::ji("bulb")` The MLE for $x_\text{min}$ is...
]
...
degree_hist_with_pl_fit(Amz, alpha_amz, dmin_amz, ylabel='')
res = plt.subplot(1, 3, 3)
degree_hist_with_pl_fit(CA, alpha_ca, dmin_ca, xlabel='', ylabel='')
plt.show()
```
...
res = plt.subplot(1, 3, 2)
degree_hist_with_pl_fit_log_log(Amz, alpha_amz, dmin_amz, ylabel='')
res = plt.subplot(1, 3, 3)
degree_hist_with_pl_fit_log_log(CA, alpha_ca, dmin_ca, xlabel='', ylabel='')
plt.show()
```
...
.insight[
`r emo::ji("bulb")` What if $p_{k_i}$ is Poisson?
]
